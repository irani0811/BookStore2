...
# 指定文件路径
output_file = 'fe/bench/performance.txt'
CACHE_FILE = Path(__file__).with_name("workload_cache.json")
# 定义函数用于写入日志信息到 txt 文件
...
class Workload:
    def __init__(self):
        ...
        self._cache_signature = {
            "book_num_per_store": self.book_num_per_store,
            "store_num_per_user": self.store_num_per_user,
            "seller_num": self.seller_num,
            "buyer_num": self.buyer_num,
            "stock_level": self.stock_level,
            "use_large_db": conf.Use_Large_DB,
        }
        self.cache_loaded = self._try_load_cache()
    ...
    def gen_database(self):
        if self.cache_loaded:
            logging.info("benchmark dataset already prepared; skipping regeneration")
            write_to_file("benchmark dataset already prepared; skip regenerate")
            return

        logging.info("load data")
        write_to_file("load data")
        ...
        write_to_file("seller data loaded.")
        ...
        write_to_file("buyer data loaded.")
    ...
    def update_stat(
        ...
        if (
            ...
            massage=("TPS_C={}, NO=OK:{} Thread_num:{} TOTAL:{} LATENCY:{} , P=OK:{} Thread_num:{} TOTAL:{} LATENCY:{}".format(
                    int(
                        self.n_new_order_ok
                        / (
                            self.time_payment / n_payment_diff
    ...
    def _try_load_cache(self) -> bool:
        ...
        try:
            with self.cache_path.open("r", encoding="utf-8") as fp:
                cached = json.load(fp)
            if cached.get("config") != self._cache_signature:
                return False
            self.uuid = cached.get("uuid")
            self.store_ids = cached.get("store_ids", [])
            self.book_ids = cached.get("book_ids", {})
            self.buyer_ids = cached.get("buyer_ids", [])
            logging.info("benchmark dataset cache hit")
            write_to_file("reuse benchmark dataset from cache")
            return bool(self.uuid and self.store_ids and self.book_ids)
        except Exception as e:
            logging.warning("failed to load benchmark cache: %s", e)
            return False
    ...
    def _save_cache(self):
        ...
        try:
            ...
            logging.warning("failed to persist benchmark cache: %s", e)